...
SEABED’s broad functionality makes it widely applicable to a variety of Python-based projects. The JAX ecosystem lets users easily implement this package in projects involving deep learning or solving differential equations, and the flexibility of the utility metric means it can be tailored to a project's individual needs. 

Since its release in 2023, SEABED has already found success in accelerating the characterization of solid-state defects [@autodisc_paper]. The generality of SEABED means that the strategies implemented here can easily be used to improve the selection and analysis of measurements in many disparate fields.
